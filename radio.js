...
var Radio = function(stations) {
  ...
  for(var i=0; i<self.stations.length; i++){
    createStationDiv(i, self.stations[i].freq, self.stations[i].title);
  }
  ...
  for (var i=0; i<self.stations.length; i++) {
    ...
    station.addEventListener('click', function(index) {
...
Radio.prototype = {
  ...
  toggleStationDisplay: function(index, state) {
    ...
    var station = document.getElementById('station' + index);
    station.style.backgroundColor = state ? 'rgba(255, 255, 255, 0.33)' : '';

    // Show/hide the "live" marker.
    var live = document.getElementById('live' + index);
    live.style.opacity = state ? 1 : 0;

    // Show/hide the "playing" animation.
    var playing = document.getElementById('playing' + index);
    playing.style.display = state ? 'block' : 'none';
  }
};
...
var radio = new Radio([
  {
    freq: '680',
    title: "News Toronto",
    src: 'http://rogers.leanstream.co/rogers/tor680.stream/icy',
    howl: null
  },
  {
    freq: '92.5',
    title: "Kiss Toronto",
    src: 'http://rogers.leanstream.co/rogers/tor925.stream/icy',
    howl: null
  },
  {
    freq: '99.9',
